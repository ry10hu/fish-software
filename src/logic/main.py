...
import start 
class files:
    conn = sqlite3.connect(start.resource_path("fish.db"))
    cursor = conn.cursor()
...
def set_image(self, name):
    ...
    if file_path:
        if file_path.split("/")[-2] != "art" or "photograph":
            ...
            shutil.copy(file_path, start.resource_path("photographs") if not art else start.resource_path("art"))
        if art:
...
def fill_fish_details( fishTextEdit, descriptionPlainTextEdit, graphicsView, name):
    ...
    if details:
        ...
        if art:
            if details[4] != None:
                setGraphicsView(graphicsView, start.resource_path(f"art/{details[4]}"))  # Assuming details[4] is the image filename
            else:
                graphicsView.setScene(None)
        else:
            if details[3] != None:
                setGraphicsView(graphicsView, start.resource_path(f"photographs/{details[3]}"))  # Assuming details[3] is the photograph filename
            else:
                graphicsView.setScene(None)
...
def make_new_fish(name, description, image_path, art_path):
    ...
    if art_path != None and art_path.endswith((".png", ".jpg", ".jpeg")):
        if art_path.split("/")[-2] != "art":
            shutil.copy(art_path, start.resource_path("art"))
    ...
    if image_path != None and image_path.endswith((".png", ".jpg", ".jpeg")):
        if image_path.split("/")[-2] != "photographs":
            shutil.copy(image_path, start.resource_path("photographs"))
